...
var vistaMidi = (msg) => {
  if (msg === 'ra') {
    ...
    // already handled 
  } else if (_.startsWith(msg, 'non')) {
    var msg = msg.substring(3);
    var note = parseInt(msg);

    log('sending midi note ' + note + ' on');

    output.sendMessage([144, note, 127]);

  } else if (_.startsWith(msg, 'noff')) {
    var msg = msg.substring(4);
    var note = parseInt(msg);
    log('sending midi note ' + note + ' off');

    output.sendMessage([128, note, 127]);
  } else {
